...
const MainView: React.FC<MainViewProps> = ({ view, searchSongs, onPlay, onDownload, activeSong, playlist, allSongs }) => {
  const renderHome = () => {
      ...
      <div className="space-y-12 animate-fadeIn">
        <section>
          <div className="flex items-end justify-between mb-8">
            <h2 className="text-3xl font-black text-white italic tracking-tight underline decoration-green-500 underline-offset-8">Fresh Releases</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-8">
            {allSongs.slice(0, 6).map(song => (
              <SongCard key={song.id} song={song} onPlay={onPlay} isActive={activeSong?.id === song.id} />
            ))}
          </div>
        </section>
...
const SongCard: React.FC<{ song: Song, onPlay: (s: Song) => void, isActive: boolean }> = ({ song, onPlay, isActive }) => (
  <div 
    onClick={() => onPlay(song)}
